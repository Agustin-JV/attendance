...
const AvChip = props => {
  ...
  const color = isClickable()
    ? props.color || (props.cAr && props.cAr.length >= 1 ? props.cAr[0] : 'grey')
    : 'grey';
  ...
  const styles = theme => ({
    chip: {
      ...
      '&:hover': {
        backgroundColor: outlined
          ? colors[color][l] + (isClickable() ? '50' : '')
          : colors[color][l + (isClickable() ? 100 : 0)]
      },
    ...
    avatar: {
      ...
      zIndex: 2,
      height: 32,
      backgroundColor: colors[color][d],
      ...
      <Chip
        avatar={
          <span>
            ...
            {loading && (
              <CircularProgress size={38} thickness={4.5} className={props.classes.progress} />
            )}
          </span>
        }
